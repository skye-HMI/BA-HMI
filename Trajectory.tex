...
Before actually a path can be drawn through a set of waypoints respectively a spline can be calculated, a value of the parameter $u$ needs to be assigned to all $n+1$ waypoints. I.e. a nondecreasing sequence of $u_0 < u_1 < ...< u_n$ must be found in order to calculate a spline for each dimension.
...
\begin{equation}
  \left|a_{max} \right| =  \frac{\| {\bf F_{res, w}}\|}{m_{tot}} = 0.96\si{\meter\per\square\second}
\end{equation}
...
\begin{equation}
\left|v_{max} \right| = \sqrt{\frac{\|{\bf F_{res,w}} \|}{\frac{1}{2}c_d \rho \pi r^2}}=\si{\meter\per\second}
\end{equation}
...
\begin{equation}
\left|\Psi_{max} \right| =  \frac{\| {\bf M_{res,w}}\|}{\left| \lambda_{max, J_{B}} \right|} = 2.06\si{\radian\per\square\second} 
\end{equation}
...
The same simulations have been conducted with wind disturbance. Wind forces based on a constant wind velocity of 1\si{\meter\per\second} in positive x-direction have been added. The disturbance starts 10 seconds after the simulations start.
...
\begin{center}
 ...
 \hline
 Controller &   & unit & \textit{Road} & \textit{Helix} & \textit{Agile} \\ \hline \hline
 Trajectory Following & Av. Dev. & $[\si{\meter}]$ & 3.227 & 1.628 & 0.838 \\
 Pure Pursuit         & Av. Dev & $[\si{\meter}]$ & 3.650 & 0.098 & 0.037 \\
 Cross Track          & Av. Dev & $[\si{\meter}]$ &  0.034 & 0.061 & 0.023 \\
