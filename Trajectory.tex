...
\section{Remarks}
To actually implement trajectory following on a real system, some further considerations have to be respected. Especially measurement noise (that was not considered in this thesis) and its influence to the system's behaviour have to be analysed. Some remarks about that as well as the case of too low continuity of a trajectory are found below.

\paragraph{Lack of Continuity}
As discussed in section \ref{sub:spline_degree}, no accurate control will be possible if the dynamic continuity of the trajectory is not high enough to consider the system dynamics. Further, discontinuous system inputs will occur which would have to be treated specially to omit exceptions on the implemented system. A simulation using ``linear splines'' is shown in figure \ref{fig:app_linear}.

\paragraph{Measurement Noise}
Precise state estimation is the cornerstone of any controller implementation. As sensors will never deliver perfect system states, the integration of measurement uncertainties should be considered already in simulation. Figure \ref{fig:app_noise_trajfoll} and \ref{fig:app_noise_crossTrack} show the effect of additive white Gaussian noise for the position and velocity feedback.
